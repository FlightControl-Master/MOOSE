...
– Cache frequently used functions and constants for better performance
...
return not grp:IsAAA() – remove AAA, keep others
end
...
return not grp:IsSAM() – remove SAM, keep others
end
...
elseif not tiresias_data.exception then
...
self:T(string_format(”%s Event = %d”, self.lid, EventData.id))
...
if self.debug then
self:I(string_format(“There are %d groups around this plane or helo!”, count))
end
...
ground:ForEachGroupAlive(
  function(grp)
    ...
    if grp:GetCoalition() ~= group_coalition 
       ...
       and not tiresias_data.exception then
      
      -- Make group visible if invisible
      if tiresias_data.invisible then
      ...
      if grp_type == "Vehicle" and tiresias_data.AIOff then
        grp:SetAIOn()
        tiresias_data.AIOff = false
      elseif SwitchAAA and grp_type == "AAA" and tiresias_data.AIOff then
        grp:SetAIOn()
...
for _, _OG in pairs(OGS or EMPTY_TABLE) do
...
BASE:I(“TIRESIAS: VEHICLE Object Added: “ .. Object:GetName())
if Object and Object:IsAlive() then
Object:SetAIOff()
...
self:I(string_format(“Overall: %d | Vehicles: %d | AAA: %d | SAM: %d”,
...
```
for _, _plane in pairs(Set or EMPTY_TABLE) do
