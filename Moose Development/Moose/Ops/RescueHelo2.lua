...
-- ===
--
-- ![Banner Image](..\Presentations\RESCUEHELO2\RescueHelo_Main.png)
--
-- # Recue Helo
--
...
-- 
--      RescueheloStennis=RESCUEHELO2:New(UNIT:FindByName("USS Stennis"), "Rescue Helo")
--      RescueheloStennis:Start()
--
-- The first line will create a new @{#RESCUEHELO2} object via @{#RESCUEHELO2.New} and the second line starts the process by calling @{#RESCUEHELO2.Start}.
-- 
-- **NOTE** that it is *very important* to define the RESCUEHELO2 object as **global** variable. Otherwise, the lua garbage collector will kill the formation for unknown reasons!
-- 
...
-- ## Takeoff Type
-- 
-- By default, the helo is spawned with running engines on the carrier. The mission designer has set option to set the take off type via the @{#RESCUEHELO2.SetTakeoff} function.
-- Or via shortcuts
-- 
--    * @{#RESCUEHELO2.SetTakeoffHot}(): Will set the takeoff to hot, which is also the default.
--    * @{#RESCUEHELO2.SetTakeoffCold}(): Will set the takeoff type to cold, i.e. with engines off.
--    * @{#RESCUEHELO2.SetTakeoffAir}(): Will set the takeoff type to air, i.e. the helo will be spawned in air near the unit which he follows.  
-- 
-- For example,
--      RescueheloStennis=RESCUEHELO2:New(UNIT:FindByName("USS Stennis"), "Rescue Helo")
...
-- 
-- If only the first spawning should happen on the carrier, one use the @{#RESCUEHELO2.SetRespawnInAir}() function to command that all subsequent spawning
-- will happen in air.
-- 
-- If the helo should no be respawned at all, one can set @{#RESCUEHELO2.SetRespawnOff}(). 
-- 
-- ## Home Base
-- 
-- It is possible to define a "home base" other than the aircraft carrier using the @{#RESCUEHELO2.SetHomeBase}(*airbase*) function, where *airbase* is
...
-- 
--    * @{#RESCUEHELO2.SetAltitude}(*altitude*), where *altitude* is the altitude the helo flies at in meters. Default is 70 meters.
--    * @{#RESCUEHELO2.SetOffsetX}(*distance*), where *distance is the distance in the direction of movement of the carrier. Default is 200 meters.
--    * @{#RESCUEHELO2.SetOffsetZ}(*distance*), where *distance is the distance on the starboard side. Default is 100 meters.
--
-- ## Rescue Operations
--
...
-- 
-- The standard "rescue zone" has a radius of 15 NM (~28 km) around the carrier. The radius can be adjusted via the @{#RESCUEHELO2.SetRescueZone}(*radius*) functions,
...
-- 
-- Once the helo reaches the crash site, the rescue operation will last 5 minutes. This time can be changed by @{#RESCUEHELO2.SetRescueDuration(*time*),
-- where *time* is the duration in minutes.
-- 
-- During the rescue operation, the helo will hover (orbit) over the crash site at a speed of 5 knots. The speed can be set by @{#RESCUEHELO2.SetRescueHoverSpeed}(*speed*),
-- where the *speed* is given in knots.
-- 
-- If no rescue operations should be carried out by the helo, this option can be completely disabled by using @{#RESCUEHELO2.SetRescueOff}().
--
-- # Finite State Machine
-- 
-- The implementation uses a Finite State Machine (FSM). This allows the mission designer to hook in to certain events.
-- 
--    * @{#RESCUEHELO2.Start}: This eventfunction starts the FMS process and initialized parameters and spawns the helo. DCS event handling is started.
--    * @{#RESCUEHELO2.Status}: This eventfunction is called in regular intervals (~60 seconds) and checks the status of the helo and carrier. It triggers other events if necessary.
--    * @{#RESCUEHELO2.Rescue}: This eventfunction commands the helo to go on a rescue operation at a certain coordinate.
--    * @{#RESCUEHELO2.RTB}: This eventsfunction sends the helo to its home base (usually the carrier). This is called once the helo runs low on gas.
--    * @{#RESCUEHELO2.Run}: This eventfunction is called when the helo resumes normal operations and goes back on station.  
--    * @{#RESCUEHELO2.Stop}: This eventfunction stops the FSM by unhandling DCS events.
--
-- The mission designer can capture these events by RESCUEHELO2.OnAfter*Eventname* functions, e.g. @{#RESCUEHELO2.OnAfterRescue}.
--
-- # Debugging
-- 
-- In case you have problems, it is always a good idea to have a look at your DCS log file. You find it in your "Saved Games" folder, so for example in
--     C:\Users\<yourname>\Saved Games\DCS\Logs\dcs.log
-- All output concerning the @{#RESCUEHELO2} class should have the string "RESCUEHELO2" in the corresponding line.
...
--     BASE:TraceLevel(1)
--     BASE:TraceClass("RESCUEHELO2")
-- 
-- To get even more output you can increase the trace level to 2 or even 3, c.f. @{Core.Base#BASE} for more details.
-- 
-- ## Debug Mode
-- 
-- You have the option to enable the debug mode for this class via the @{#RESCUEHELO2.SetDebugModeON} function.
...
--- Create a new RESCUEHELO2 object. 
-- @param #RESCUEHELO2 self
...
-- @return #RESCUEHELO2 RESCUEHELO2 object.
function RESCUEHELO2:New(carrierunit, helogroupname)

  -- Inherit everthing from FSM class.
  local self = BASE:Inherit(self, FLIGHTGROUP:New(helogroupname, false)) -- #RESCUEHELO2
  ...
  -- Increase ID.
  _RESCUEHELO2ID=_RESCUEHELO2ID+1
  
  -- Unique ID of this helo.
  self.uid=_RESCUEHELO2ID
  
  -- Save self in static object. Easier to retrieve later.
  self.carrier:SetState(self.carrier, string.format("RESCUEHELO2_%d", self.uid) , self)
  
  -- Set unique spawn alias.
  self.alias=string.format("%s_%s_%02d", self.carrier:GetName(), self.helogroupname, _RESCUEHELO2ID)
  
  -- Log ID.
  self.lid=string.format("RESCUEHELO2 %s | ", self.alias)
  ...
  -- @function [parent=#RESCUEHELO2] Rescue
  -- @param #RESCUEHELO2 self
  ...
  -- @function [parent=#RESCUEHELO2] RTB
  -- @param #RESCUEHELO2 self
  ...
  -- @function [parent=#RESCUEHELO2] Returned
  -- @param #RESCUEHELO2 self
...
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetLowFuelThreshold(threshold)
...
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetHomeBase(airbase)
...
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetRescueZone(radius)
...
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetRescueHoverSpeed(speed)
...
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetRescueDuration(duration)
...
-- @param #RESCUEHELO2 self
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetRescueOn()
  self.rescueon=true
  return self
end
...
-- @param #RESCUEHELO2 self
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetRescueOff()
  self.rescueon=false
  return self
end
...
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetTakeoff(takeofftype)
...
-- @param #RESCUEHELO2 self
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetTakeoffHot()
...
-- @param #RESCUEHELO2 self
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetTakeoffCold()
...
-- @param #RESCUEHELO2 self
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetTakeoffAir()
...
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetAltitude(alt)
...
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetOffsetX(distance)
...
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetOffsetZ(distance)
...
-- @param #RESCUEHELO2 self 
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetRespawnOn()
  self.respawn=true
  return self
end
...
-- @param #RESCUEHELO2 self 
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetRespawnOff()
  self.respawn=false
  return self
end
...
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetRespawnOnOff(switch)
...
-- @param #RESCUEHELO2 self
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetRespawnInAir()
...
-- @return #RESCUEHELO2 self
function RESCUEHELO2:SetFollowTimeInterval(dt)
...
function RESCUEHELO2:IsRescuing()
...
function RESCUEHELO2:GetUnitName()
...
function RESCUEHELO2:OnEventLand(EventData)
...
-- @param #RESCUEHELO2 self
-- @param Core.Event#EVENTDATA EventData Event data.
function RESCUEHELO2:_OnEventCrashOrEject(EventData)
...
function RESCUEHELO2:onafterStart(From, Event, To)
  ...
  local text=string.format("Starting Rescue Helo Formation v%s for carrier unit %s of type %s.", RESCUEHELO2.version, self.carrier:GetName(), self.carriertype)
  self:I(self.lid..text)
  ...
  self.flightgroup=FLIGHTGROUP:New(self.helo)  --Ops.FlightGroup#FLIGHTGROUP
...
-- @param #string Event Event.
-- @param #string To To state.
function RESCUEHELO2:onafterStatus(From, Event, To)
...
-- @param #string Event Event.
-- @param #string To To state.
function RESCUEHELO2:onafterRun(From, Event, To)
...
function RESCUEHELO2:_TaskRTB()
  ...
  DCSScript[#DCSScript+1] = string.format('local myhelo    = mycarrier:GetState(mycarrier, \"RESCUEHELO2_%d\") ', self.uid) -- Get the RESCUEHELO2 self object.
...
function RESCUEHELO2:onafterRescue(From, Event, To, RescueCoord)
  ...
  --local TaskRTB=self.helo:TaskFunction("RESCUEHELO2._TaskRTB", self)
...
function RESCUEHELO2:onafterRTB(From, Event, To, airbase)
...
function RESCUEHELO2:onafterReturned(From, Event, To, airbase)
...
-- @param #string Event Event.
-- @param #string To To state.
function RESCUEHELO2:onafterStop(From, Event, To)
...
function RESCUEHELO2:RouteRTB(RTBAirbase, Speed)
